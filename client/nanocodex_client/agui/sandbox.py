...
LANGUAGES_INSTRUCTIONS = (
    ...
    "— a bare return or trailing expression is DISCARDED (empty output). There is "
    "no require(); fs, fetch and the engine helpers are already global.\n"
...
SKILLS_INSTRUCTIONS = (
    ...
    "engine helpers are already global. So print results: "
    "console.log(JSON.stringify(await fs.readdir('/opt/languages/skills'))).\n"
    ...
    "SUPERPOWERS: general software-engineering methodology skills (obra/"
    "superpowers) are bundled READ-ONLY under "
    "/opt/languages/skills/superpowers/<name>/SKILL.md — brainstorming, "
    "test-driven-development, systematic-debugging, writing-plans, "
    "verification-before-completion, receiving/requesting-code-review, and "
    "more (fs.readdir('/opt/languages/skills/superpowers') for the full list). "
    "Before non-trivial work, consult the relevant one and follow it. These "
    "assume Claude-Code tools you don't have — substitute: their `Skill` tool "
    "→ fs.readFile the SKILL.md (reading it IS loading it); `TodoWrite` → "
    "update_plan; `Task`/subagents → do the work yourself unless sub-agent "
    "tools are present; `Read/Write/Edit/Bash` → the run_js fs.* on /work and "
    "/codex-home/skills. See superpowers/README.md.\n"
