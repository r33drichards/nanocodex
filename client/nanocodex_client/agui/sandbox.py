...
SKILLS_INSTRUCTIONS = (
    ...
    "web service — try it). To hand a program to a ComputerCraft turtle you "
    "cannot type into, PUT it to the mutable paste store, e.g. in run_js: "
    "await fetch('https://paste-production.up.railway.app/<id>', {method:'PUT', "
    "body: luaSource}) (create-or-overwrite; GET the same url returns it raw). "
    "The turtle then loads it once with `wget "
    "https://paste-production.up.railway.app/<id> prog` and runs `prog`; "
    "editing the slot hot-reloads it. See the poll-ccraft-lua skill for the "
    "full turtle harness. V8 heap persistence is disabled on this thread — "
    "persist cross-call state in /work."
)
