...
class ImageInputTest(unittest.TestCase):
    ...
    def test_arbitrarily_many_images(self):
        ...
        self.assertTrue(
            all(o["url"].startswith("data:image/jpeg;base64,") for o in out if o["type"] == "image")
        )

    def test_only_trailing_user_message_after_last_assistant(self):
        msgs = [
            _user("old", "u0"),
            AssistantMessage(id="a0", role="assistant", content="reply"),
            _user("new", "u1"),
        ]
    ...
    def test_data_value_already_a_data_url(self):
        src = InputContentDataSource(
            type="data", value="data:image/png;base64,QUJD", mime_type="image/png"
        )
